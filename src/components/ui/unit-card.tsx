...
const UnitCard = ({ className, sliderWrapperClassName }: Props) => {
  ...
  const url = `/unit/1?${searchParams.toString()}`
  return (
    <Link
      href={url}
      className={cn(
      ...
      <div
        ...
        <FavoriteButton className=" group absolute left-2 top-3  z-[1]" />
      </div>
      <div className="flex grow gap-2 p-4 max-lg:items-end  max-lg:gap-2">
        <div>
          <div>
            ...
            <dd className="text-sm text-gray-500">$240,000 / ليلة</dd>
          </div>
          <div>
            <dt className="sr-only">العنوان</dt>

            <dd className="text-lg font-medium">اسم الوحدة او الفندق</dd>
          </div>
          <div>
